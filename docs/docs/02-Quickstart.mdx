...
plugins {
    ...
    id("dev.kikugie.stonecutter") version "9.2.1"
}
...
:::info Stonecutter 0.7+
Gradle's built-in tasks (`build`, `publishMods`, etc.) now run for every Stonecutter node automatically. Stonecraft still provides its `chiseled*` wrappers so older CI pipelines keep working, but you can rely on the vanilla tasks if you prefer. For more details, see [our blog post](/blog/native-tasks-deprecation).
:::
