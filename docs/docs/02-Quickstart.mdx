...
:::info Gradle tasks
With the Stonecutter 0.9 line shown above, use the native Gradle tasks (`build`, `publishMods`, etc.). Stonecraft also keeps the `chiseled*` wrappers for existing workflows. See [our blog post](/blog/native-tasks-deprecation) for the task naming background.
:::
